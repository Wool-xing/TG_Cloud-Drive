...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">仪表盘</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">最近操作日志</h2>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {(data.recentLogs ?? []).slice(0, 20).map(log => (
                  <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-2.5 text-gray-600 dark:text-gray-400 hidden lg:table-cell">
                      <span className="text-xs truncate max-w-[140px] block">{log.nodeName ?? '—'}</span>
                    </td>
                    <td className="px-4 py-2.5 text-gray-500 dark:text-gray-500 text-xs">{formatDate(log.createdAt)}</td>
                  </tr>
                ))}
                {(!data.recentLogs || data.recentLogs.length === 0) && (
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-gray-400 dark:text-gray-600 text-sm">暂无日志</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">存储使用 Top 10</h2>
          ...
          <div className="p-4 space-y-3">
            ...
            {(!data.topStorageUsers || data.topStorageUsers.length === 0) && (
              <p className="text-center text-gray-400 dark:text-gray-600 text-sm py-4">暂无数据</p>
            )}
          </div>
        </div>
      </div>
